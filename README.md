...
The RangeMap get() method finds the appropriate key by dividing the input parameter by the interval of the RangeMap's keys, and then multiplying the result by the interval to calculate the starting value of the corresponding Range Object. The Object id is passed as a key to the superclass’s get() method, allowing any key/value pair, even those that aren’t explicitly initialized, to be searched by reference.
